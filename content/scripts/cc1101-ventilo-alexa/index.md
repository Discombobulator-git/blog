---
title: "CC1101 : pilotage d'un ventilateur de plafond (Raspberry Pi Zero W)"
date: 2026-07-20T10:00:00+02:00
tags: ["RF", "SDR", "CC1101", "Raspberry Pi", "domotique"]
summary: "Scripts de production pour reproduire une telecommande de ventilateur de plafond avec un CC1101 et un Raspberry Pi Zero W."
---

Contexte complet dans l'article [Faire parler un ventilateur de plafond]({{< ref "/posts/cc1101-ventilo-alexa/index.md" >}}). Ces scripts sont la version qui tourne aujourd'hui en production, portée sur Raspberry Pi Zero W (pigpio, wave DMA).

Câblage (header 40 broches standard) :
...
## `check_cc1101.py` — vérification du câblage SPI

Lit les registres `PARTNUM` et `VERSION` du CC1101. Sur une puce authentique, `VERSION` vaut généralement `0x14` (parfois `0x04` ou `0x03` selon le lot). Si vous obtenez `0x00` ou `0xFF`, c'est un problème de câblage (MISO/MOSI inversés, CS non connecté, alimentation ou masse manquante), pas une vraie réponse du composant.

[Télécharger check_cc1101.py](check_cc1101.py)
...
## `cc1101_tx_ventilo_pigpio.py` — transmission OOK via pigpio

Configure le CC1101 puis transmet le code correspondant à un bouton. Chaque trame réelle fait 33 bits (voir l'article pour l'histoire de ce bit caché), générée en wave DMA pigpio pour un timing garanti à la microseconde. Usage :

```bash
python3 cc1101_tx_ventilo_pigpio.py <bouton>
# boutons disponibles : lumiere, onoff, v1, v2, v3, v4, v5, v6, inversion
```

[Télécharger cc1101_tx_ventilo_pigpio.py](cc1101_tx_ventilo_pigpio.py)
...
## `ventilo_api.py` — pont HTTP (Flask)

Expose chaque bouton comme une route HTTP (`GET /<bouton>`), pour être appelé depuis Home Assistant (`rest_command`) ou n'importe quel autre système domotique. Lance le script de transmission en sous-processus dans le venv dédié.

```bash
python3 ventilo_api.py
# service disponible sur http://<ip-du-pi>:8765/
```

[Télécharger ventilo_api.py](ventilo_api.py)
